...
# Install packages
#install("opencv-python") # CH needs to be install_package
#install("requests")
 
# Note: 'tkinter' usually doesn't need to be installed via pip
# CH: TkInter is included in Vanilla Python
...
EDAMAM_APP_ID = '????'
EDAMAM_APP_KEY = '????'
# CH please make this more convenient for me, either make a file
# called api_keys.py with this
# EDAMAM_APP_ID = "w3yeuibrvgyrbawh"
# EDAMAM_APP_KEY = "ma8unocob e98wrywiov"
# (make sure api_keys.py is in your .gitignore!)
# change the above to
#from api_keys.py import EDAMAM_APP_ID, EDAMAM_APP_KEY
# with this we can both have local key files and the same code
# will work for both of us.
# Alternative:  use .env file instead (google python dotenv)
...
class MealTrackerApp:
    ...
    def preview_camera(self):
        if self.previewing:
            ...
            if ret:
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # CH why?
