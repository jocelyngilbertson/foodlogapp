...
class MealTrackerApp:
    ...
    def delayed_key_release(self, event):
        # Cancel the previous schedule if it exists
        if self.autocomplete_timer is not None:
            self.root.after_cancel(self.autocomplete_timer)
        # Schedule autocomplete_food_name to be called after 0.5 seconds
        self.autocomplete_timer = self.root.after(500, self.autocomplete_food_name)
    ...
    def setup_widgets(self):
        ...
        self.food_name_entry.bind('<KeyRelease>', self.delayed_key_release)
        ...
        self.autocomplete_listbox.grid(row=3, column=1, columnspan=2, padx=5, pady=5, sticky='ew')
        ...
        # CH use grid for the following as well?
        ...
        self.scrollbar = ttk.Scrollbar(self.bottom_frame, orient=tk.VERTICAL, command=self.food_listbox.yview)  
    ...
    def autocomplete_food_name(self):
        ...
        while url:
            ...
            if response.status_code == 200:
                ...
                # Check if there is a 'next' link to continue paging
                if '_links' in data and 'next' in data['_links']:
                    url = data['_links']['next']['href']
                    print(page)
                    
                    page += 1
                    if page > 100:  # CH limit to 100 pages
                        url = False
                else:
                    url = False
        return None
    ...
    def on_autocomplete_select(self, event):
        ...
        if selection:
            ...
            # CH I think its nice to select multiple items 
            self.food_name_entry.delete(0, tk.END)
